...
export function initCubixEnvironment() {
    ...
    const Z_LEVELS = {
        outer: -180, // Adjusted: Not zooming out as extremely far
        inner: -75   // Adjusted: Not zooming in as narrow
    };
    ...
    function initOmniBridge() {
        ...
        const handleOmniSubmit = () => {
            if(!omniInput.value.trim()) return;
            const div = document.createElement('div');
            ...
            div.style.background = "rgba(161, 193, 209, 0.1)";
            div.innerText = omniInput.value;
            chatFeed.appendChild(div);
            omniInput.value = "";
            chatFeed.scrollTop = chatFeed.scrollHeight;
        };
    ...
    function initVaultBridge() {
        ...
        // Librarian Chat Target (in vault-col-right)
        ...
        const handleLibrarianSubmit = () => {
            if(!librarianInput.value.trim()) return;
            const div = document.createElement('div');
            div.style.marginBottom = "10px";
            div.style.color = "#A1C1D1";
            div.innerHTML = `<b>You:</b> ${librarianInput.value}`;
            librarianFeed.appendChild(div);
