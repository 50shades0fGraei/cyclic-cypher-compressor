...
# ═══════════════════════════════════════════════════════════════════
#  CyberDNA: EliteBook Randall Build — Full Compile Pipeline
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║   CyberDNA: Randall Compile Pipeline — Starting...    ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo ""

mkdir -p "$BUILD_DIR"

# ─── STEP 1: Generate Randall RSA-4096 Keys ─────────────────────────────────
echo "[ STEP 1 ] Generating RSA-4096 Randall Keys..."
if [ ! -f "$BUILD_DIR/randall_private.key" ]; then
    openssl genrsa -out "$BUILD_DIR/randall_private.key" 4096
    openssl rsa -in "$BUILD_DIR/randall_private.key" -pubout -out "$BUILD_DIR/randall_public.key"
    openssl req -new -x509 \
        -key "$BUILD_DIR/randall_private.key" \
        -out "$BUILD_DIR/randall_root.crt" \
        -days 3650 \
        -subj "/C=UC/ST=CYBERDNA/L=TESSERACT/O=LUJAN_AGI/CN=Randall_Root"
    echo "[✓] Keys generated: randall_private.key + randall_root.crt"
else
...
echo ""
echo "[ STEP 4 ] Signing CubixOS-loader.efi with Randall Root..."
...
sbsign \
    --key  "$BUILD_DIR/randall_private.key" \
    --cert "$BUILD_DIR/randall_root.crt" \
...
cp "$BUILD_DIR/randall_root.crt"        "$EFI_BUNDLE/EFI/CyberDNA/"
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║          RANDALL BUILD COMPLETE                        ║"
echo "╠══════════════════════════════════════════════════════════╣"
echo "║  Artifacts in: ./build-output/                          ║"
echo "║  • randall_private.key  (KEEP SECRET)                 ║"
echo "║  • randall_root.crt     (Enroll in EliteBook BIOS db) ║"
...
echo "║        → Boot from CyberDNA entry → Randallty achieved║"
echo "╚══════════════════════════════════════════════════════════╝"
echo ""
