...
#   /dev/nvme0n1p3  →  rest    Sovereign DNA Vault          ext4
# ═══════════════════════════════════════════════════════════════════════
...
SOVEREIGN_LABEL="CyberDNA-Tesseract-OS"
...
# Using Arch Linux ARM (lightest sovereign base)
# Alternative: debootstrap ubuntu-arm64
if command -v pacstrap &>/dev/null; then
    # Arch Linux ARM route
    pacstrap "$MOUNT_ROOT" base linux linux-firmware networkmanager python
    ...
    # Install kernel and python inside chroot
    arch-chroot "$MOUNT_ROOT" apt-get install -y linux-image-generic-arm64 python3 networkmanager
else
...
# ─── STEP 4: Copy Tesseract-OS Stack ─────────────────────────────────────────
echo ""
echo "[ STEP 4 ] Deploying CyberDNA / Tesseract-OS stack..."
...
# ─── STEP 8: Disable Windows Boot Manager (optional but sovereign) ────────────
echo ""
...
DRIVER_SRC="$(dirname "$0")/../../EliteBook-Sovereign-Build/build-output/npu-retention-driver.ko"
...
# ─── STEP 10: Deploy Sovereign Autostart ─────────────────────────────────────
echo ""
echo "[ STEP 10 ] Configuring Tesseract-OS autostart on login..."
cat > "$MOUNT_ROOT/etc/profile.d/cyberdna-launch.sh" << 'AUTOSTART'
#!/bin/bash
# Auto-launch Tesseract-OS interface after sovereign login
if [ "$(tty)" = "/dev/tty1" ] && [ "$USER" = "sovereign" ]; then
    echo ""
    echo "  CyberDNA Tesseract-OS — Sovereign Boot Complete"
    echo "  Identity: Randall Lujan / LUJAN_AGI_PRIME"
    echo ""
    cd /home/sovereign/cyclic-cypher-compressor
    python3 EliteBook-Sovereign-Build/sovereign-interface/biometrics-bridge/bridge.py && \
    python3 cubix_os.html 2>/dev/null || \
    python3 -m http.server 8080 &
    xdg-open http://localhost:8080/cubix_os.html 2>/dev/null || \
    echo "Tesseract-OS ready at http://localhost:8080"
fi
AUTOSTART
